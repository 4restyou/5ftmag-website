...
2. **내 애플리케이션 → 애플리케이션 추가하기**
   - 앱 이름: `5ft.mag`
   - 사업자명: `4rest` (또는 본인 이름)
3. 생성 후 좌측 **앱 설정 → 앱 키**에서 **REST API 키** 복사
...
6. **제품 설정 → 카카오 로그인 → 동의항목**:
   - **닉네임** — 필수 동의
   - **프로필 사진** — 선택 동의 또는 필수
   - **카카오계정(이메일)** — 선택 동의 (이메일 가져올 때 필요. 옵션)

7. **제품 설정 → 보안 → Client Secret**:
   - **코드 생성** → **상태 ON** 으로 변경
...
4. **Client Secret**: Kakao Developers에서 생성한 Client Secret
5. **Save**
...
- "댓글 0" 영역이 보이면 ✅ 연결 성공
- "Google" / "Kakao" 버튼 클릭 → 로그인 → 댓글 작성 가능
...
1. 한 번 사이트에 Google/Kakao로 로그인 (계정이 만들어짐)
...
→ `js/supabase-config.js`의 url/anonKey가 아직 placeholder. 6단계 다시 확인.
...
**Q. Kakao 로그인 후 "OAuth callback failed"**
→ Kakao Developers 사이트의 Redirect URI와 Supabase Provider 설정이 일치하는지 확인. 그리고 카카오 로그인이 ON 상태인지 확인.
...
**Q. 편집부 배지가 안 떠요**
→ 8단계의 UPDATE를 실행한 후, 사이트에서 한 번 로그아웃 → 재로그인. 또는 페이지 새로고침.

**Q. 스팸이 너무 많아요**
→ Supabase의 Auth → Rate Limits 에서 초당 인증 요청 수 줄이기. 또는 모더레이션 모드로 전환 (본 가이드에서는 자동 게시지만, 사후 모더레이션은 9단계 SQL로 가능).
