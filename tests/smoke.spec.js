  ...
  const result = await page.evaluate(() => ({
    ...
    brokenImages: [...document.images]
      .filter(img => {
        ...
        // Netlify Image CDN 은 정적 서버 CI 에선 404 — 스킵.
        if ((img.currentSrc || img.src).includes('/.netlify/images')) return false;
        if (img.loading === 'lazy') {
...
test('카톡 인앱형 짧은 공유 링크들이 모바일에서 asset을 잃지 않는다', async ({ page }) => {
  ...
  for (const target of paths) {
    ...
    const result = await page.locator(target.imageScope).evaluate(scope => ({
      ...
      brokenImages: [...scope.querySelectorAll('img')]
        .filter(img => {
          ...
          if (img.closest('.lightbox')) return false;
          if (img.complete && img.naturalWidth > 0) return false;
          // Netlify Image CDN 은 정적 서버 CI 에선 404 (실배포에서만 동작) — 스킵.
          if ((img.currentSrc || img.src).includes('/.netlify/images')) return false;
          if (img.loading === 'lazy') {
            const rect = img.getBoundingClientRect();
            const inViewport = rect.bottom > 0 && rect.top < window.innerHeight;
    ...
    // /.netlify/images 는 Netlify Image CDN (런타임 동적 리사이즈). 정적 서버
    // CI 환경에선 404 가 정상 — 실배포에서만 처리됨. 다른 도메인 외부 의존도
    // 마찬가지로 제외.
    expect(failed.filter(url => !/supabase|cdn\.jsdelivr|favicon|\/\.netlify\/images/i.test(url)), `${target.path} failed requests`).toEqual([]);
  }
});
