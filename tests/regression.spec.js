...
test('김현아 SVG 로고는 다크모드에서 반전되어 보인다', async ({ page }) => {
  await page.goto('/');
  const homeLogoBox = await page.locator('.footer-partner img[src$="logo-kimhyuna-lockup.svg"].logo-light').boundingBox();
  expect(homeLogoBox.width).toBeGreaterThan(40);
  expect(homeLogoBox.height).toBeGreaterThan(10);

  await page.evaluate(() => {
    document.documentElement.dataset.theme = 'dark';
  });
  const logo = page.locator('.footer-partner img[src$="logo-kimhyuna-lockup.svg"].logo-dark');
...
test('모바일 홈은 추천 필름과 현재 호 참여를 중심으로 압축해 보여준다', async ({ page }) => {
