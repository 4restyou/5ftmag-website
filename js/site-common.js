...
(function () {
  ...
  const recordPageView = telemetry.recordPageView || function () {};
  const trackEvent = telemetry.trackEvent || function () {};
  ...
  function init() {
    ...
    // 정보 구조 개선 후 실제 이용 흐름을 확인하기 위한 최소 이벤트.
    document.addEventListener('click', function (e) {
      const link = e.target.closest('.main-nav a, .mobile-nav a, .nav-more-menu a');
      if (!link) return;
      trackEvent('nav_clicked', {
        label: String(link.textContent || '').trim().slice(0, 40),
        area: link.closest('.mobile-nav') ? 'mobile' : (link.closest('.nav-more-menu') ? 'more' : 'desktop'),
      });
    });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { init(); revealAdminSubnavCurrent(); });
  } else {
    init();
