...
async function shareListing(id) {
  ...
  try {
    ...
    showShareToast('링크 복사 완료');
  } catch (_) {
    // 3) execCommand fallback (구형 WebView)
    ...
    ta.remove();
    showShareToast(ok ? '링크 복사 완료' : '복사 실패 — 주소창에서 직접 복사해주세요');
  }
}
  ...
  $('mktDetailCard').querySelectorAll('[data-action]').forEach(el => {
    el.addEventListener('click', async (e) => {
      ...
      if (a === 'cycle') {
        ...
        if (error) { el.disabled = false; return alert('상태를 바꾸지 못했어요. 새로고침 후 다시 시도해 주세요. (' + error.message + ')'); }
        r.status = next;
        await loadList();
        renderDetail(r);
        $('mktDetailModal').classList.add('open');
      ...
      if (a === 'delete') {
        ...
        if (error) { el.disabled = false; return alert('매물을 삭제하지 못했어요. 권한이나 네트워크 상태를 확인해 주세요. (' + error.message + ')'); }
        if (r.storage_paths?.length) await db().market.removePhotos(r.storage_paths);
        closeDetail();
        return loadList();
      }
      if (a === 'report') {
        if (!STATE.user) return alert('신고는 로그인 후에 가능해요. 로그인하면 보던 매물로 다시 돌아옵니다.');
        const reason = prompt('신고 사유를 적어주세요 (300자 이내):', '');
        if (!reason) return;
        const { error } = await db().market.report(r.id, reason);
        if (error) return alert('신고를 접수하지 못했어요. 잠시 뒤 다시 시도해 주세요. (' + error.message + ')');
        alert('신고가 접수되었습니다. 편집부에서 검토할게요.');
      }
    });
  });
}
  ...
  row.querySelectorAll('input[type=file]').forEach(el => {
    el.addEventListener('change', async () => {
      ...
      try {
        ...
        alert(err.message || '사진을 준비하지 못했어요. 다른 사진으로 다시 시도해 주세요.');
      }
    });
  });
