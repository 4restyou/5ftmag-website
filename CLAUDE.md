...
Claude Code 가 이 저장소에서 작업할 때 따라야 할 정책. **이 파일 하나로 충분해야 한다** — 프로젝트 배경, 스택, 지켜야 할 기준, 끝내기 전 확인 항목을 여기서 다 잡는다. 세부 실행 절차는 `.claude/skills/` 스킬과 `docs/` 로 링크한다.
...
| 경로 | 내용 |
|---|---|
| `index.html`, `stories/`, `authors/`, `films.html`, `labs.html`, `market.html`, `books.html`, `shop.html` | 공개 페이지 (정적 HTML) |
| `admin/` | 편집부 관리 화면 (카탈로그·기사·이북·메시지 등의 source of truth UI) |
| `js/` | 페이지별 vanilla JS. `js/db-client.js` = 유일한 Supabase 접근 계층(`window.MagDB.*` 도메인 함수만 노출). 공통 로직은 `js/site-*.js`(common·telemetry·pwa)·`js/db/*.js` 모듈로 분리 |
| `css/` | 페이지별 CSS + `tokens.css`(색·타이포·간격 토큰) |
| `data/*.json` | 대부분 **빌드 산출물**(아래 "데이터 소스"). 단 `data/site-shell.json` 은 공통 내비/푸터의 **원본 config** — 페이지별로 손대지 말 것 |
| `db/baseline.sql` + `db/baseline-manifest.json` | 초기 스키마 **전체 베이스라인**(생성물). 복구 시 이걸 먼저 적용한 뒤 마이그레이션을 시간순으로 (`docs/database-recovery.md`) |
| `supabase/migrations/*.sql` | 베이스라인 이후의 증분 DB 마이그레이션 (replay-safe) |
| `supabase/functions/<name>/index.ts` | Deno 엣지 함수 (자체 인증) |
| `scripts/build-*.mjs` | DB dump → 정적 JSON/페이지 생성 |
| `scripts/` (유틸) | `validate-assets` · `qa-smoke` · `bump-version` · `db-audit` · `sync-site-shell`(공통 셸 주입) · `build-db-baseline` · `content-health` |
...
`.claude/skills/` 는 세션에 자동 로드된다. 해당 상황이면 **먼저 스킬을 따른다** (아래 정책과 스킬이 충돌하면 CLAUDE.md 우선):

- **`ship`** — 표준 배포 루프(브랜치 → PR → CI 폴링 → squash 머지 → 후속 워크플로우 확인). "배포해 / 머지까지 진행해" 및 코드를 라이브로 보내는 모든 작업.
- **`new-article`** — 기사 제작 파이프라인(원고 추출 → 이미지 처리 → 페이지 생성 → `data/stories.json` 등록).
- **`add-lab`** — 현상소(labs) 데이터 관리.
...
- main 에 **직접 커밋 금지**. 모든 작업은 새 브랜치 `claude/<feature-or-fix-slug>` → PR → **squash 머지** (커밋 제목 = PR 제목 + ` (#NN)`).
...
- 형식: `type(scope): 한 줄 요약` (한국어). 예: `fix(security): …`, `feat(story): …`, `docs(relay): …`. 본문에 무엇을/왜.
- **모델 ID·이름을 커밋 메시지/PR/코드/산출물에 넣지 않는다** (채팅 답변에만).

## 완료의 정의 (Definition of Done)

끝났다고 보고하기 전에 해당하는 것을 **실제로 돌려서** 통과를 확인한다. "고쳤다"가 아니라 "확인했다"로 말한다.

- **항상** — `node scripts/validate-assets.mjs` (exit 0: 자산 참조 + 단일 캐시버전 가드).
- **JS 수정** — `node --check <파일>` 문법 확인. 로직 바뀌면 `npm run test:unit` (vitest).
- **발행 글/데이터 변경** — `node scripts/qa-smoke.mjs` (rss·sitemap 동기화 드리프트).
- **기사 초고를 쓴 뒤** — `npm run writing:check stories/<파일>.html` (자연도 등급 A 를 목표로. CI 게이트는 아니다. 아래 "글쓰기 규칙" 참고).
...
DB 관련 작업 전에 반드시 인지. 진단 이력·할 일은 `docs/maintenance.md`, 계약 현황은 `docs/db-contract-audit.md`.

- **프로덕션 프로젝트는 하나** — ref `pucpqsfwqouqohwsvmnd` (사이트 JS·워크플로우에 노출된 공개정보). db-deploy·functions-deploy 는 전부 이 프로젝트로 나간다.
- **prod 스키마 확정 사항(2026-07 기준)**: `profiles_public` 뷰 + `profiles.is_editor` 컬럼이 **prod 에 존재한다** (db-contract-audit + 실제 마이그레이션 적용으로 확인됨). **편집부 권한 체크는 `profiles.user_id + is_editor` 패턴으로 통일**한다 — `role='admin'` 등 다른 패턴을 새로 도입하지 않는다 (prod 컬럼 불일치로 실패한 이력 있음).
- **`db-deploy.yml` 은 `SUPABASE_ACCESS_TOKEN` 시크릿에 의존.** 만료되면 마이그레이션 자동 적용이 조용히 실패한다 (`Unauthorized`). 마이그레이션 머지 후엔 db-deploy 성공 여부를 확인하고, 실패면 Studio SQL Editor 직접 적용 + 시크릿 갱신 안내(`docs/secrets-rotation.md`).
- **DB 재현 순서**: `db/baseline.sql`(초기 스키마 전체 스냅샷) 먼저 → `supabase/migrations/**`(증분) 시간순. `comments`/`likes`/`market_listings`/`profiles_public`/`reader_submissions` 등 마이그레이션에 정의 없는 객체는 베이스라인에 포함돼 있다. 복구 절차는 `docs/database-recovery.md`, 현황 대조는 `node scripts/db-audit.mjs`. **베이스라인은 생성물** — 스키마를 바꾸면 개별 스키마 파일/`db/baseline-manifest.json` 수정 후 `npm run db:baseline` 로 재생성한다(CI 의 `db:baseline:check` 가 드리프트를 막는다).
...
- **실패한 마이그레이션은 기록되지 않는다.** 새 파일을 만들지 말고 **같은 파일을 수정해 재적용**한다 (트랜잭션 롤백이라 부분 적용도 없다).

## 보안 / RLS 모델 (개인정보 다루는 실서비스)

이 사이트는 실제 개인정보(독자 투고, 장터 판매자 연락처, 구매 내역)를 다룬다. DB·엣지 함수를 건드릴 때 아래를 전제로 판단한다. 감사 반영 마이그레이션 예: `supabase/migrations/20260710000001_security_hardening_pii.sql`.

- **anon 키는 공개 설계다.** `js/db-client.js` 에 role=anon JWT 가 하드코딩돼 있고, 공격자는 이 키 + 요청 주소 + 클라이언트 JS 를 다 본다. **보안은 키 비밀이 아니라 RLS + 엣지 함수 인증에 있다.**
- **PostgREST 는 베이스 테이블을 직접 노출한다.** 연락처를 가린 "마스킹 뷰"를 만들어도, 베이스 테이블 SELECT 정책이 넓으면 공격자가 베이스를 직접 질의해 우회한다. **민감 컬럼은 베이스 테이블 정책 자체를 본인/편집부로 좁혀야** 한다.
- **공개 브라우징은 뷰로 한다.** 공개 뷰는 `security_invoker = false`(정의자 권한)로 두어 베이스 RLS 를 우회해 안전한 컬럼만 노출한다. 베이스 테이블을 좁히기 전에 **뷰를 먼저 정의자 권한으로 고정**한다(순서 반대면 화면이 깨진다).
- **`is_editor` 체크는 호출자 본인 프로필만 읽는다** (`EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND is_editor)`). 그래서 `profiles` 베이스를 "본인만" 으로 좁혀도 다른 테이블의 편집부 정책은 깨지지 않는다(단, profiles 자체 SELECT 정책엔 편집부 절을 넣지 말 것 — RLS 재귀).
- **엣지 함수는 자체 인증한다** (`getUser(token)` 로 검증). `--no-verify-jwt` 로 배포되므로 함수 안에서 반드시 토큰을 확인하고, 민감 동작은 요청자 신원을 대조한다(예: `ebook-redeem` 의 주문자 이름·연락처 대조).
- **스토리지 버킷은 `public=true`** — `/object/public/<bucket>/<path>` 는 정책과 무관하게 서빙된다. 표시용 이미지는 이 경로로 나가므로 안전하지만, `/object/list`(열거)·인증 다운로드는 소유자 한정 정책으로 좁힌다.
- **비밀키는 채팅에 받지 않는다.** `supabase secrets set KEY=...`(또는 Dashboard → Edge Functions → Secrets) 를 안내한다. 공개키(anon key, Store ID, Channel Key)는 클라이언트 하드코딩 허용.

## 캐시 버스트

정적 자산(`css/*.css`, `js/*.js`) 을 수정하면 그 파일을 참조하는 모든 HTML 의 `?v=YYYYMMDD-feature` 쿼리를 일괄 갱신한다. **`node scripts/bump-version.mjs <css/x.css|js/x.js> <YYYYMMDD-feature>` 를 쓴다** — 전 페이지 + `scripts/templates/` 까지 한 번에 통일한다. 수동 sed 로 일부만 고치지 않는다.

- **같은 자산은 전 페이지 단일 버전**이어야 한다. 일부만 갱신하면 버전이 갈라져 캐시 중복 다운로드 + 스타일 불일치가 생긴다 (tokens.css 가 4개 버전으로 갈라졌던 사례). `validate-assets.mjs` 의 단일 버전 가드가 CI 에서 이를 실패시킨다.
...
`npm run writing:check <파일>` 로 AI 작문 흔적을 센다. 두 곳에서 가져와 하나로 합쳤고 둘 다 MIT 다.

| 출처 | 가져온 것 |
|---|---|
| [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) | A~J 분류. S1·S2 목록의 뼈대 |
| [DaleSeo/korean-skills](https://github.com/DaleSeo/korean-skills) 의 humanizer | `H-*` 로 표시한 항목과 자연도 등급 (KatFishNet 논문 기반) |

여기에 우리가 겪어서 따로 넣은 K-1·K-2·K-3 을 더했다. **CI 게이트가 아니다.** 문체는 사람이 판단할 몫이고, 수치는 어디를 다시 볼지 알려 줄 뿐이라 종료 코드는 늘 0 이다.

**대상은 사람이 읽는 산문이다.** 기사, 초고, 공지, 소개문. 기술 문서(`docs/*.md`)에 돌리면 상태 표시 이모지와 표의 콜론까지 잡혀 수치가 의미를 잃는다.

**자연도 등급으로 본다.** 발행 전 A 를 목표로 한다.

| 등급 | 기준 | 뜻 |
|---|---|---|
| **A** | S1 0건 · 임계치 넘긴 S2 0~2종 | 사람이 쓴 것처럼 읽힌다 |
| **B** | S1 1~2건 또는 S2 3~4종 | 대체로 자연스럽고 흔적이 조금 남았다 |
| **C** | S1 3건 이상 또는 S2 5종 이상 | AI 흔적이 뚜렷하다 |
| **D** | S1 5건 이상 + S2 6종 이상 | 다시 쓰는 편이 빠르다 |

S2 는 발생 횟수가 아니라 **임계치를 넘긴 항목 수**로 센다. "할 수 있다" 가 스무 번 나와도 한 종이다. 화면에서 `←` 가 붙은 줄이 그것이다.

- **S1 은 대체로 고친다.** "~에 대해", "~에 있어", "가지고 있다", 이중 피동, "그녀/그것/그들", 이모지, 상투 종결구.
- **의미는 건드리지 않는다.** 수치·날짜·고유명사·직접 인용은 그대로 둔다. 인용문 안의 어색한 표현은 원문이므로 고치지 않는다.
- **고친 양이 원문의 30% 를 넘으면 멈추고 다시 본다.** 문체를 고치다 내용을 바꾸고 있다는 신호다. 실제로 2001 기사에서 대조 구문을 다듬다 "엑타크롬은 필름이 아니다" 라는 사실 오류를 만든 적이 있다.

우리가 따로 넣은 항목 셋:

| 항목 | 내용 | 기준 |
|---|---|---|
| **K-1** | "A 가 아니라 B" 대조 구문 | 스무 문장당 1회를 넘기지 않는다 |
| **K-2** | 캡션이 본문 문장을 그대로 되풀이 | 0개 |
| **K-3** | 의문문 없이 평서문만 이어짐 | 긴 글이면 한두 번은 섞는다 |

K-1 이 가장 자주 나온다. 다음 토큰의 범위를 좁혀 주고 별 내용이 없어도 통찰처럼 보이게 만들기 때문이다. 2001 기사 초고에 15곳 있었다. 고치는 방법은 결론만 남기기, 의문문으로 바꾸기, 문장 나누기, 대구로 풀기다. **다만 논지 자체가 대조인 자리는 남긴다.**

**제목 안(〈〉《》「」)의 대시와 콜론은 세지 않는다.** 공식 전시명·작품명은 원문이라 우리가 고칠 대상이 아니다.

검사기가 과하게 잡을 때가 있다. 열거의 쉼표를 번역투로 오인해 기사 절반이 걸린 적이 있고, humanizer 의 「지시관형사」를 "이/그/저" 로 잘못 읽어 오탐 규칙을 만들 뻔한 적도 있다(실제로는 "해당·본·동" 같은 격식체를 잡는 규칙이다). **수치가 이상하면 규칙부터 의심하고 표본을 직접 읽는다.**

**적용 범위는 2026-08-23 이후 기사부터다.** 그 전 기사 50편에는 대시가 27편에 남아 있지만 손대지 않기로 했다(운영자 결정). 발행된 글을 문체 때문에 소급해 고치면 독자가 읽은 것과 달라지고, 얻는 것에 비해 건드릴 것이 너무 많다. `--all` 은 현황을 보는 용도이지 일감 목록이 아니다. **지난 기사를 일괄 윤문하지 말 것.** 사실 오류나 운영자 요청이면 물론 개별로 고친다.

## 외부 서비스 연동 체크리스트

외부 SDK·API(결제, 지도 등)를 붙일 때 순서대로 (실행 절차서: `add-external-service` 스킬):
...
5. 새 엣지 함수를 만들면 `functions-deploy.yml` 이 그 함수를 배포하는지 확인한다 (전 디렉토리 스캔이라 보통 자동).
...
| curl 로 남의 데이터가 나옴 | 베이스 테이블 RLS 정책이 넓음 — 마스킹 뷰만 믿지 말 것("보안/RLS 모델" 참고) |
