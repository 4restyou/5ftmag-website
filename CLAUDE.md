...
- **기사 초고를 쓴 뒤** — `npm run writing:check stories/<파일>.html` (S1 을 0 으로. CI 게이트는 아니다. 아래 "글쓰기 규칙" 참고).
...
`npm run writing:check stories/<파일>.html` 로 AI 글쓰기 흔적을 센다. 분류 체계는 [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) (MIT) 를 옮겨 왔다. **CI 게이트가 아니다.** 문체는 사람이 판단할 몫이고, 수치는 어디를 다시 볼지 알려 줄 뿐이다.

- **S1 은 대체로 고친다.** "~에 대해", "~에 있어", "가지고 있다", 이중 피동, "그녀/그것/그들", 이모지, 상투 종결구.
- **S2 는 개수를 보고 판단한다.** 한두 번은 자연스러운 한국어인 경우가 많다.
...
검사기가 과하게 잡을 때가 있다. 열거의 쉼표를 번역투로 오인해 기사 절반이 걸린 적이 있다. **수치가 이상하면 규칙부터 의심하고 표본을 직접 읽는다.**

**적용 범위는 2026-08-23 이후 기사부터다.** 그 전 기사 50편에도 A-1·A-7 이 남아 있지만 손대지 않기로 했다(운영자 결정). 발행된 글을 문체 때문에 소급해 고치면 독자가 읽은 것과 달라지고, 얻는 것에 비해 건드릴 것이 너무 많다. `--all` 은 현황을 보는 용도이지 일감 목록이 아니다. **지난 기사를 일괄 윤문하지 말 것.** 사실 오류나 운영자 요청이면 물론 개별로 고친다.
