...
| `js/` | 페이지별 vanilla JS. `js/db-client.js` = 유일한 Supabase 접근 계층(`window.MagDB.*` 도메인 함수만 노출) |
| `css/` | 페이지별 CSS + `tokens.css`(색·타이포·간격 토큰) |
| `data/*.json` | **빌드 산출물** (원본 아님 — 아래 "데이터 소스" 참고) |
| `supabase/migrations/*.sql` | DB 마이그레이션 (replay-safe). **prod 스키마의 완전한 원본은 아님** |
| `supabase/functions/<name>/index.ts` | Deno 엣지 함수 (자체 인증) |
| `scripts/build-*.mjs` | DB dump → 정적 JSON/페이지 생성 |
| `scripts/validate-assets.mjs`, `qa-smoke.mjs`, `bump-version.mjs`, `db-audit.mjs` | 검증·유틸 |
...
| `docs/` | `maintenance.md` · `db-contract-audit.md` · `relay-setup.md` · `secrets-rotation.md` |
...
- **CI = `.github/workflows/test.yml`** (워크플로우 이름 "CI", 체크 잡 이름 `validate`). 이 잡 하나가 순서대로 실행: 자산검증(`validate`) → vitest 단위 → qa-smoke(rss·sitemap 드리프트) → (코드 파일 변경 시에만) Playwright 스모크. data/img/문서만 바뀐 PR 은 Playwright 를 건너뛴다.
- 자동 파이프라인(머지 후):
...
- **DB 작업** — `node scripts/db-audit.mjs` 로 클라이언트 참조 ↔ 스키마 계약 확인.
- **전체 한 방** — `npm run qa` = validate + qa-smoke. (CI 는 여기에 vitest·Playwright 를 더한다.)
...
- **마이그레이션 폴더가 prod 스키마의 완전한 원본이 아니다.** `comments`/`likes`/`market_listings`/`profiles_public`/`reader_submissions` 등은 prod 에 있으나 `supabase/migrations/**` 에 정의가 없다(초기 스키마는 `db/*.sql` 에서 수동 적용됨). 레포만으로 DB 재현 불가. `node scripts/db-audit.mjs` 로 현황 확인.
